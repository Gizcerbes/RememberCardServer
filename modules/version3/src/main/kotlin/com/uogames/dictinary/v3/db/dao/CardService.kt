...
object CardService {
    ...
    private fun getQuery(
        ...
        return CardTable
            .join(first, JoinType.LEFT, CardTable.idPhrase, first[PhraseTable.id])
            .join(second, JoinType.LEFT, CardTable.idTranslate, second[PhraseTable.id])
            .slice(CardTable.columns)
            .selectAll()
            .buildWhere(text, langFirst, langSecond, countryFirst, countrySecond, first, second)
            .orderBy(
                first[PhraseTable.length] to SortOrder.ASC,
                second[PhraseTable.length] to SortOrder.ASC,
    ...
    fun getView(
        ...
        //CardView.toView(CardViewEntity[it[CardTable.id]])
        CardView.fromRow(it)
    }
